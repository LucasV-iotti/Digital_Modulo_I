...
// Update6: integra correção da mensagem duplicada + fluxo Abordagem → Negociação → CadOK → Tabulação → Finalização
// + jogador escolhe a resposta de proposta (sem mensagens automáticas do agente)
...
const MISSAO_I = {
  ...
  nodes:{
    ...
    // Jogador escolhe a resposta de PROPOSTA
    proposta_resposta:{
      customerVariants:{
...
function proceedToNode(key){ 
  ...
  // 🔧 Evita duplicar mensagem ao entrar em 'proposta_resposta' após autorreply da negociação
...
function startScenario(){ 
  ...
  typing(true);
  setTimeout(()=>{ typing(false); renderCustomerMessage(first); renderOptions(nodeByKey(state.currentNodeKey).options); updateFeedbackPanel(); }, 520);
}
...
// ===== Auto-reply de negociação → vai para proposta_resposta (SEM agente automático) =====
function handleNegotiationAutoReply(opt) {
  ...
  setTimeout(() => {
    ...
    // Evita duplicar a mensagem ao entrar em proposta_resposta
    state.skipNextCustomerMsg = true;
