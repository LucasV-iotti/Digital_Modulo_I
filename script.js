...
// Padrão: Modo Difícil SEM seletor. Opções mais similares e com pegadinhas, ordem aleatória e penalidade mais rígida por demora.
...
const state = { currentScenarioIndex:0, currentNodeKey:'start', satisfaction:50, metrics:{ empatia:0, resolucao:0, tempo:0, total:0 }, optionShownAt:0 };
...
function updateMetricsUI(){
  document.querySelector('#mEmpatia').textContent = state.metrics.empatia;
  document.querySelector('#mResolucao').textContent = state.metrics.resolucao;
  document.querySelector('#mTempo').textContent = state.metrics.tempo;
}
...
function renderCustomerMessage(text){ if(!text) return; const el=document.createElement('div'); el.className='msg customer'; el.innerHTML=`${escapeHtml(applyTokens(text))}<div class="meta"><span>${nowTime()}</span><span class="status">• recebido</span></div>`; const w=document.querySelector('#chatWindow'); w.appendChild(el); w.scrollTop=w.scrollHeight; }
function renderAgentMessage(text){ const el=document.createElement('div'); el.className='msg agent'; el.innerHTML=`${escapeHtml(applyTokens(text))}<div class="meta"><span>${nowTime()}</span><span class="status" data-status>✓</span></div>`; const w=document.querySelector('#chatWindow'); w.appendChild(el); w.scrollTop=w.scrollHeight; setTimeout(()=>{ const s=el.querySelector('[data-status]'); if(s) s.textContent='✓✓'; },650); }
function typing(on){ const t=document.querySelector('#typing'); if(t) t.classList.toggle('hidden', !on); const hs=document.querySelector('#headerStatus'); if(hs) hs.textContent = on ? 'online • digitando…' : 'online'; }

function renderOptions(options=[]) { const wrap=document.querySelector('#options'); wrap.innerHTML=''; const list = shuffle(options.slice(0,4)); list.forEach((opt,i)=>{ const b=document.createElement('button'); b.className='option'; b.addEventListener('click', ()=>chooseOptionByRef(opt)); const kbd=`<span class="kbd">${i+1}</span>`; b.innerHTML=`<span>${escapeHtml(applyTokens(opt.text))}</span>${kbd}`; wrap.appendChild(b); }); const first=wrap.querySelector('.option'); if(first) first.focus(); state.optionShownAt = performance.now(); }

function chooseOptionByRef(opt){ renderAgentMessage(opt.text); if(opt.effects) applyEffects(opt.effects); const dt=(performance.now()-state.optionShownAt)/1000; if(dt>4){ state.metrics.tempo+=2; } else if(dt>2){ state.metrics.tempo+=1; } updateMetricsUI(); const nextKey=opt.next||state.currentNodeKey; const nextNode=nodeByKey(nextKey); if(nextNode?.agentAuto){ typing(true); setTimeout(()=>{ typing(false); renderAgentMessage(nextNode.agentAuto); proceedToNode(nextKey); },650);} else { proceedToNode(nextKey);} }

function proceedToNode(key){ state.currentNodeKey=key; const node=nodeByKey(key); const tone= state.satisfaction>=75?'happy': state.satisfaction>=50?'neutral': state.satisfaction>=25?'annoyed':'rude'; const msg=node.customerVariants? node.customerVariants[tone]:null; if(msg){ typing(true); setTimeout(()=>{ typing(false); renderCustomerMessage(msg); renderOptions(node.options); },520);} else { renderOptions(node.options);} if(node.end){ setTimeout(()=>{ finishScenario(); setScreen('#end-screen'); },800);} }
